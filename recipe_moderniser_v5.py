...
Incorporating a converter to convert the units to mls then g- (component 7 & 8)
Version 5
...
for recipe_line in full_recipe:
    ...
    if num_spaces > 1:  # Item has both unit and ingredient
        ...
        if amount[1]:  # food_dictionary gives gms per 250 ml so conversion factor is 250
            amount_2 = general_converter(amount[0], ingredient, food_dictionary, 250)

            # If 'true' the ingredient is in food_dictionary, so converts it to grams
            if amount_2[1]:
                modernised_recipe.append("{} grams {}".format(amount_2[0], ingredient))
