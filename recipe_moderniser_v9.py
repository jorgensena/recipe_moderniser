...
Changes to formality - variable names and line lengths - to meet PEP8
Version 9
...
def not_blank(question, error_msg, allow_num):
    ...
    while not valid:
        ...
        if not response or number is True:  # print error for blank name or
            # digits
            print(error)
...
def get_scale_factor():
    ...
    while not keep_scale:
        ...
        if factor < 0.25:
            print("Scale Factor: {}".format(factor))
...
def general_converter(amount_3, lookup, dictionary, conversion_factor):
    if lookup in dictionary:
        factor = dictionary.get(lookup)
        amount_3 *= float(factor) / conversion_factor
        converted = True
    else:
        converted = False
    return [amount_3, converted]
...
for recipe_line in full_recipe:
    ...
    if num_spaces > 1:  # Item has both unit and ingredient
        ...
        if amount[1]:  # food_dictionary gives gms per 250 ml so conversion
            ...
            # to grams
            if amount_2[1]:
                modernised_recipe.append("{:.0f} grams {}".format(amount_2[0],
                                                                  ingredient))
